...
from moco.pose_res_adv import get_pose_net_adv
...
class Adv_Model(nn.Module):
    def __init__(self, dim=128, K=65536, m=0.999, T=0.2):
        ...
        # create the encoders
        self.encoder_q = get_pose_net_adv()
        self.encoder_k = get_pose_net_adv()
    ...
    def forward(self, im_q, im_k, meta_q, meta_k, adv_train=True):
        """
        Input:
        ...
        if adv_train:
            ...
            q_inv, q_var, joints_q = self.encoder_q(im_q, out_joints=True)  # queries: NxC
            ...
            with torch.no_grad():  # no gradient to keys
                ...
                k_inv, k_var = self.encoder_k(im_k, out_joints=False)  # keys: NxC
            ...
            return logits_inv, labels_inv, logits_var, labels_var, [l_pos_inv, l_neg_inv, l_pos_var, l_neg_var], joints_q

        else:
            _, _, joints = self.encoder_q(im_q, out_joints=True)
            return joints
